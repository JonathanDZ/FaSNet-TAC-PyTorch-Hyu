...
class FaSNet_origin(FaSNet_base):
    ...
    def forward(self, input, num_mic):
        ...
        if num_mic.max() == 0:
            ...
            print("헤헤헤헤",bf_signal.shape)
            print('안녕',bf_signal[0,:,:num_mic[0]].shape)
...
class FaSNet_TAC(FaSNet_base):
    ...
    def forward(self, input, num_mic):
        ...
        #print(self.encoder(all_mic_context.view(-1, 1, self.context*2+self.window)).shape)
        ...
        mic_context = torch.cat([all_mic_context.view(batch_size*nmic, 1, seq_length, self.context*2+self.window)]*self.num_spk, 1)  # B*nmic, nspk, L, 3*win
